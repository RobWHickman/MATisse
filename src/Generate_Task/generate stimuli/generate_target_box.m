...
function target_box = generate_target_box(parameters, stimuli, hardware, correct)
if parameters.targeting.static
    target_box_box_length = stimuli.bidspace.bidspace_info.height * parameters.targeting.startsize;
elseif ~parameters.targeting.static
    minimum_size = stimuli.bidspace.bidspace_info.height * 0.1;
    maximum_size = stimuli.bidspace.bidspace_info.height * parameters.targeting.startsize;
    
    target_box_box_length = ((maximum_size - minimum_size) - ((maximum_size - minimum_size) * (1/ (1 + exp(1) ^ (-(correct-50)/20))))) + minimum_size;
end
