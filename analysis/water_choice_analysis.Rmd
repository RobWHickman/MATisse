...
bundle_bidding <- task_data[!is.na(fractal_bid)] %>%
  .[, .(budget_water_perc, bundle_water_perc, fractal_bid)] %>%
  .[, fractal_bids := mean(fractal_bid), by = .(budget_water_perc, bundle_water_perc)] %>%
  .[, trials := .N, by = .(budget_water_perc, bundle_water_perc)] %>%
  .[, bundle_liquid := 1.2 - (bundle_water_perc * 1.2)] %>%
  .[, budget_liquid := 1.2 - (budget_water_perc * 1.2)] %>%
  .[, .(budget_liquid, bundle_liquid, fractal_bids, trials)]
